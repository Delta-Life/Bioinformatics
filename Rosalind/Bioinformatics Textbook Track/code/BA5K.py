# Find a Middle Edge in an Alignment Graph in Linear Space
# https://rosalind.info/problems/ba5k/

from utilities import get_file, get_answer_file, load_blosum62, load_protein_dictionary

def get_half_matrix(score_matrix, str1, str2, penalty):
    protein_dict = load_protein_dictionary()
    cost = [[0]*(len(str1)+1) for _ in range(len(str2)+1)]
    match_table = [[0]*(len(str1)) for _ in range(len(str2))]
    for i in range(len(cost)):
        cost[i][0] = -penalty * i
    for i in range(len(cost[0])):
        cost[0][i] = -penalty * i
    for i in range(len(match_table)):
        for j in range(len(match_table[0])):
            match_table[i][j] = score_matrix[protein_dict[str2[i]]][protein_dict[str1[j]]]
    for i in range(1, len(str2)+1):
        for j in range(1, len(str1)+1):
            cost[i][j] = max(cost[i-1][j]-penalty, cost[i][j-1]-penalty, cost[i-1][j-1]+match_table[i-1][j-1])
    
    return cost

def get_middle_edge(str1, str2, score_matrix, penalty):
    middle = (len(str2))//2+1
    forward_half = get_half_matrix(score_matrix, str2[:middle], str1, penalty)
    backward_half = get_half_matrix(score_matrix, str2[middle:][::-1], str1[::-1], penalty)
    middle_cost = [forward_half[i][-1] + backward_half[len(backward_half)-1-i][-1] for i in range(len(forward_half))]
    max_cost = max(middle_cost)
    j = middle
    i = middle_cost.index(max_cost)
    result = [[i, j]]

    if forward_half[i][j-1] - penalty == forward_half[i][j]:
        result.insert(0, [i-1, j])
    elif forward_half[i-1][j] - penalty == forward_half[i][j]:
        result.insert(0, [i, j-1])
    else:
        result.insert(0, [i-1, j-1])

    return result


with get_file() as file:
    str1 = file.readline().rstrip()
    str2 = file.readline().rstrip()

with get_answer_file() as file:
    BLOSUM62 = load_blosum62()
    for i in get_middle_edge(str1, str2, BLOSUM62, 5):
        print(tuple(i), end=' ', file=file)